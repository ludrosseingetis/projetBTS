...
<body>
        ...
        <style>.mothaut{
            ...
            <a href=""><input type="button" value="Demandes affecté a mon équipe" class="demandeekpc"></a>
  ...
  <?php //  <a href="demandemoi.php"><input type="button" value="Demandes affecté a mon équipe" class="demandeekp"></a>
    ...
    ?>
        ...
        $requete7 = $pdo->prepare("SELECT id_équipe FROM technicien WHERE id_technicien = $idtech");
        $requete7->execute();
        $result7 = $requete7->fetch(PDO::FETCH_ASSOC);



        $id_equipe = $result7['id_équipe'];
            $requete =  "SELECT * FROM tickets WHERE tickets.id_equipe = $id_equipe";
...
if (isset($_SESSION['id'])) {
    try {
        ...
        $requete1 = $pdo->prepare("SELECT id_équipe FROM technicien WHERE id_technicien = $idtech");
        $requete1->execute();
        $result = $requete1->fetch(PDO::FETCH_ASSOC);
        
        if ($result) {
            $id_equipe = $result['id_équipe'];
        
            // Fetch the tickets based on id_equipe
            $requete2 = $pdo->prepare("SELECT * FROM tickets WHERE tickets.id_equipe = $id_equipe");
       ...
       ?><br><tr> <td><h3>Id du ticket</h3></td> <td><h3>titre du ticket</h3></td><td><h3>commentaire</h3></td><td><h3>id de l'utilisateur</h3></td><td><h3>id du technicien</h3></td><td><h3>Importance</h3></td><td><h3>Etat</h3></td><td><h3>Modifier</h3></td><?php 
        while($enreg = $requete2->fetch(PDO::FETCH_ASSOC))
        {
            $po = $enreg["id_ticket"];
