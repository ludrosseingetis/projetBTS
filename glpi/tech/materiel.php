...
<body>
        ...
        <style>.mothaut{
            ...
            <button class="dropdown-btn">Gestion des demandes 
  ...
  <?php //  <a href="demandemoi.php"><input type="button" value="Demandes affecté a mon équipe" class="demandeekp"></a>
    ...
    ?>
        ...
        $code=$_GET['num'];
            $requete = "SELECT * FROM materiel where id_user = $code";
...
if (isset($_SESSION['id'])) {
    try {
       ...
       ?><br><n>               Voici le matériel de l'utilisateur numéro </n><?php echo $code;
...
$code=$_GET['num'];
    ...
    $requete2 = "SELECT * FROM materiel where id_user = $code";  
...
$resultat= $pdo->query($requete2);
        ...
        <br><tr> <td><h3>Id du matériel</h3></td> <td><h3>Nom du matériel</h3></td><td><h3>Code barre</h3></td><td><h3>Statut</h3></td><td><h3>Id de l'utilisateur</h3></td><td><h3>Grade</h3></td><td><h3>ID Technicien</h3></td><td><h3>Modifier</h3></td><?php
        ...
        <tr>
            <td>
           <?php echo $enreg["id_materiel"]; ?>
            </td>
            <td>
                
                <?php
                echo $enreg["nom_materiel"];
                ?>
            </td>
            <td>
                <?php
                echo $enreg["Code_barre"];
                ?>
            </td>
            <td>
                <?php
            ...
            <td><?php echo "<a href='infouser.php?num=".$enreg['id_user']."' style='color:blue'>".$enreg['id_user']."</a>" ?>
            </td>
          
            <td>
            <?php echo "<a href='infouser.php?num=".$enreg['id_user']."' style='color:blue'>supprimer</a>" ?>
        </td>
            
                
        </tr>
        <?php } ?>
        <form method="POST" action="cngmatos.php">
        <tr>

            <td><select class="matos" value="matos" name="matos"><option> Ajouter un matériel</option>

            <?php $matos='SELECT * from materiel where materiel.id_user <>'.$code ;
            $res = $pdo->query($matos);
            while($enrega = $res->fetch(PDO::FETCH_ASSOC))
        {
           ?><option class='matos' value="<?php echo $enrega['id_materiel']; ?>"><?php echo $enrega['nom_materiel']; ?></option><?php
